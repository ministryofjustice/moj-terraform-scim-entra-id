...
class TestMemberSync(unittest.TestCase):
    ...
    def test_sync_group_members_user_already_exists(self, mock_boto_client, mock_get_user_id):
        ...
        members = [{'userPrincipalName': 'user1@example.com', 'givenName': 'User', 'surname': 'One'}]

        sync_group_members(identity_center_client, 'mocked_identity_store_id', group_info, members, 'mocked_group_name', dry_run=False)
    ...
    @patch('function.app.get_identity_center_user_id_by_username')
    @patch('function.app.boto3.client')
    def test_sync_group_members_user_does_not_exist(self, mock_boto_client, mock_get_user_id):
        identity_center_client = MagicMock()
        mock_boto_client.return_value = identity_center_client
    ...
    @patch('function.app.get_identity_center_user_id_by_username')
    @patch('function.app.boto3.client')
    def test_sync_group_members_user_does_not_exist_dry_run(self, mock_boto_client, mock_get_user_id):
        identity_center_client = MagicMock()
        mock_boto_client.return_value = identity_center_client
        ...
        sync_group_members(identity_center_client, 'mocked_identity_store_id', group_info, members, 'mocked_group_name', dry_run=True)
    ...
    @patch('function.app.get_identity_center_user_id_by_username')
    @patch('function.app.boto3.client')
    def test_sync_group_members_user_already_in_group(self, mock_boto_client, mock_get_user_id):
        identity_center_client = MagicMock()
        mock_boto_client.return_value = identity_center_client
    ...
    @patch('function.app.get_identity_center_user_id_by_username')
    @patch('function.app.boto3.client')
    def test_sync_group_members_error_handling(self, mock_boto_client, mock_get_user_id):
        identity_center_client = MagicMock()
        mock_boto_client.return_value = identity_center_client
        ...
        identity_center_client.create_user.side_effect = ClientError(
            {"Error": {"Code": "InternalFailure", "Message": "An internal error occurred"}},
            "CreateUser"
        )
    ...
    def test_remove_obsolete_users(self, mock_boto_client, mock_get_user_id):
        ...
        mock_paginator.paginate.return_value = iter([{
            'GroupMemberships': [{'MembershipId': 'mocked_membership_id', 'MemberId': {'UserId': 'mocked_user_id'}}]
        }])
    ...
    def test_delete_unused_users(self, mock_boto_client, mock_get_user_id):
        ...
        identity_center_client.describe_user.return_value = {
            ...
            "Emails": [{"Value": "user1@example.com", "Type": "EntraId", "Primary": True}]
        }
